...
def main():
    ...
    larg_max = max(PORTES[f["porte"]][0] for f in fichas)
    alt_max = max(PORTES[f["porte"]][1] for f in fichas)
    celula_l = (larg_max + 4) * ESCALA
    celula_a = (alt_max + 4) * ESCALA
    rotulo = 46
    ...
    for i, f in enumerate(fichas):
        ...
        for j, (direcao, coluna) in enumerate(QUADROS):
            q = quadro(im, lg, at, direcao, coluna).resize((lg * ESCALA, at * ESCALA), Image.NEAREST)
